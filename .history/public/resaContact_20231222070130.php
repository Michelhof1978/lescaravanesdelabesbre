...
<form class="needs-validation" id="formulaire" novalidate action="#" method="POST">
  ...
  for (let i = 1; i <= nombreEnfants; i++) {
    ...
    const dateEuropeenne = convertToCustomDate(inputDate.value);
    ...
    <script>
        function convertToCustomDate(dateString) {
  // Découper la date en trois parties
  const [year, month, day] = dateString.split('-');

  // Retourner la date au format 00-00-0000
...
<script>
    ...
    // Vérifier si la date est au format AAAA-MM-JJ
    const regex = /^\d{4}-\d{2}-\d{2}$/;
    ...
    // Convertir la date au format européen pour la vérification
    const europeanDate = convertToEuropeanDate(dateString);
    if (!isValidDate(europeanDate)) {
        return false;
    }

    // La date est valide au format AAAA-MM-JJ et au format européen
    return true;
}
...
function checkDates() {
    for (const champ of document.querySelectorAll('input[pattern]')) {
        ...
        if (champ.id !== 'dateNaissanceEnfant1' && !isValidDateFormat(champ.value)) {
            event.preventDefault();
            champ.focus();
            alert('Veuillez saisir une date valide au format AAAA-MM-JJ.');
            return false;
        }
    }
}
