...
// Récupérer tous les éléments de menu qui ont un sous-menu
var dropdowns = document.querySelectorAll('#menu li ul');

// Ajouter un événement de survol de la souris à chaque élément de menu
dropdowns.forEach(function(dropdown) {
  ...
  parent.addEventListener('mouseover', function() {
    ...
    for (var i = 0; i < 10; i++) {
      ...
      star.className = 'star';
      star.style.left = Math.floor(Math.random() * parent.offsetWidth) + 'px';
      star.style.top = Math.floor(Math.random() * parent.offsetHeight) + 'px';
  ...
  parent.addEventListener('mouseout', function() {
    ...
    // Supprimer les étoiles scintillantes
    var stars = parent.querySelector('.stars');
    if (stars) {
      parent.removeChild(stars);
    }
  });
});
