...
// Récupère le canvas et définit ses dimensions
...
// Définit les variables pour les étoiles
var stars = [];
var numStars = 200;
var radius = 1;
...
for (var i = 0; i < numStars; i++) {
  ...
  var depth = Math.random() * canvas.width;
  var star = {
    x: x,
    y: y,
    depth: depth,
...
function drawStars() {
  ...
  // Boucle à travers les étoiles
  for (var i = 0; i < numStars; i++) {
    var star = stars[i];

    // Calcule la position 2D de l'étoile
    var x = (star.x - canvas.width/2) * (focalLength / star.depth) + canvas.width/2;
    var y = (star.y - canvas.height/2) * (focalLength / star.depth) + canvas.height/2;
...
// Met à jour les étoiles en fonction de la position de la souris
function updateStars(event) {
  for (var i = 0; i < numStars; i++) {
    var star = stars[i];

    // Calcule la distance entre l'étoile et la souris
    var dx = star.x - event.clientX;
    var dy = star.y - event.clientY;
    var distance = Math.sqrt(dx*dx + dy*dy);

    // Calcule la taille de l'étoile en fonction de la distance
    star.radius = radius * (1 - distance / canvas.width);

    // Empêche la taille de l'étoile de devenir négative
    if (star.radius < 0) {
      star.radius = 0;
    }
  }
